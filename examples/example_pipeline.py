...
axes[0].fill_between(x[n_train:], conf_int[:, 0], conf_int[:, 1],
                     alpha=0.1, color='b')
# axes[0].title('Actual test samples vs. forecasts')
...
pipeline.update(newly_observed, maxiter=10)  # take 10 steps for new samples

# Calling predict will now predict from newly observed values
new_preds, new_conf_int = pipeline.predict(still_test.shape[0],
                                           return_conf_int=True)

x2 = np.arange(data.shape[0])
n_trained_on = n_train + newly_observed.shape[0]
updated_train = data[:n_trained_on]

axes[1].plot(x2[:n_trained_on], updated_train, alpha=0.75)
axes[1].scatter(x2[n_trained_on:], new_preds, alpha=0.4, marker='o')
axes[1].scatter(x2[n_trained_on:], still_test, alpha=0.4, marker='x')
axes[1].fill_between(x2[n_trained_on:], new_conf_int[:, 0], new_conf_int[:, 1],
                     alpha=0.1, color='b')
# axes[1].title('Actual test samples vs. forecasts')
